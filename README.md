...
* **自动识别终点** ：引入了 `no_new_content_count`。当程序请求到第 12 页时，搜索结果为空，`new_added` 为 0，程序会标记一次。连续两次抓不到新内容，程序就判定搜索已结束并退出。
* **链接深度过滤** ：在 `_collect_links_from_html` 里，通过 `if '/doc/' in full_url` 确保只抓文章，通过 `if 'page=' not in full_url` 剔除了分页按钮的干扰。
* **防止重复抓取** ：使用了 `full_url.split('?')[0]`。很多网站会在 URL 后面加随机参数防止缓存，这会导致同一个网页被当成不同的链接。分割掉参数后，`set()` 就能精准地过滤掉重复网页。
* **安全防护** ：加入了随机的 `User-Agent` 切换和 `random.uniform` 随机等待，降低被 Kommersant 暂时封锁 IP 的概率。

#### v3自适应停止机制，且链接去掉时间戳

我们需要把**“人机校验（反爬拦截）” **与** “内容抓取完毕” **这两个状态区分开，并在检测到拦截时引入一个** “深度休眠+断点重试”**的机制。

1. **特征识别** ：增加对“人机验证”页面的判断（通常通过状态码 `403`、`429` 或 HTML 中的特定关键字识别）。
2. **原地重试** ：如果触发人机，程序不再 `break`，而是 **地等待 60-120 秒** ，然后 **重新尝试抓取当前这一页** 。
3. **防止丢包** ：只有在请求成功（200 OK）且确实没发现文章时，才累计 `no_new_content_count`。
